...
let map = (element: t('a), i) =>
  ...
  | (CloseBracket(_) | Digit(_) | Magnitude(_)) as e => Unresolved(e, i)
  ...
  | NLog({base}) => UnresolvedFunction(NLog({base: base}), i)
  | Sum({start, end_}) => UnresolvedFunction(Sum({start, end_}), i)
  | Product({start, end_}) => UnresolvedFunction(Product({start, end_}), i)
